...
for "_i" from 0 to 7 do {
    ...
    private _edge = _display displayCtrl (9270 + _i);
    _back ctrlSetTextColor [0.018,0.038,0.049,0.96];
    _edge ctrlSetTextColor [0.20,0.68,0.86,0.62];
};

private _title = if (_category in ["quick","main"]) then {"QUICK ACTIONS"} else {toUpper _category};
private _detail = if (_category in ["quick","main"]) then {
    "Press 1-8 or click an action. Highest-priority actions are shown first."
} else {
...
if (_slot >= 0 && {_actionIndex < count _actions}) then {
    ...
    (_display displayCtrl (9260 + _slot)) ctrlSetTextColor [0.045,0.22,0.30,0.98];
    (_display displayCtrl (9270 + _slot)) ctrlSetTextColor [0.35,0.86,1,1];
};

(_display displayCtrl 9201) ctrlSetText _icon;
private _titleCtrl = _display displayCtrl 9202;
_titleCtrl ctrlSetFontHeight ([0.034,0.027] select ((count _title) > 18));
_titleCtrl ctrlSetText _title;
(_display displayCtrl 9203) ctrlSetStructuredText parseText format ["<t align='center' color='#bdd3dc'>%1</t>",_detail];
