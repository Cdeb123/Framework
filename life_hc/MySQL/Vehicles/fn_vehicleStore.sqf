...
private _unitCharacterUid = [_unit getVariable ["characterUID",getPlayerUID _unit]] call HC_fnc_mresString;
...
if !(_vehicleCharacterUid isEqualTo _unitCharacterUid) exitWith {
    [1,"That vehicle belongs to a different character."] remoteExecCall ["life_fnc_broadcast",_ownerID];
    life_garage_store = false;
    _ownerID publicVariableClient "life_garage_store";
};

if !(_vehicleSide isEqualTo _unitSide) exitWith {
    [1,"That vehicle belongs to a different role garage."] remoteExecCall ["life_fnc_broadcast",_ownerID];
    life_garage_store = false;
    _ownerID publicVariableClient "life_garage_store";
};
